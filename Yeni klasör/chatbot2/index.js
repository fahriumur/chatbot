    ...
    query.get().then(function(querySnapshot) {
        if (querySnapshot.empty) {
            ...
            appointmentRef.doc(appointmentData.id).set(appointmentData).then(function() {
                ...
                handleResults(["Randevunuzu kaydederken bir hata oluştu. Lütfen tekrar deneyin.","Başka bir isteğiniz var mı?"], ["Evet", "Hayır"]);
                closeOptions();
            });
        } else {
            // Randevu dolu
        ...
        handleResults(["Randevunuzu kontrol ederken bir hata oluştu. Lütfen tekrar deneyin.","Başka bir isteğiniz var mı?"], ["Evet", "Hayır"]);
        closeOptions();
    });
}
...
function queryAppointmentsByID(appointmentID) {
    ...
    appointmentRef.get().then(function(querySnapshot) {
        if (querySnapshot.empty) {
            handleResults(["Belirttiğiniz ID ile eşleşen bir randevu bulunamadı.","Başka bir isteğiniz var mı?"], ["Evet", "Hayır"]);
        } else {
        ...
        handleResults(["Randevularınızı sorgularken bir hata oluştu. Lütfen tekrar deneyin.","Başka bir isteğiniz var mı?"], ["Evet", "Hayır"]);
        closeOptions();
    });
}
        ...
        if (querySnapshot.empty) {
            ...
            querySnapshot.forEach(function(doc) {
                doc.ref.delete().then(function() {
                    handleResults(["Randevunuz başarıyla iptal edildi.","Başka bir isteğiniz var mı?"], ["Evet", "Hayır"]);
                }).catch(function(error) {
                    console.error("Error removing document: ", error);
                    handleResults(["Randevunuzu iptal ederken bir hata oluştu. Lütfen tekrar deneyin.","Başka bir isteğiniz var mı?"], ["Evet", "Hayır"]);
                });
            });
        }
    }).catch(function(error) {
